...
<?php if($config['Embedded Script'] == 'off') { ?>
	<?php if(!empty($config['Header Image'])) { ?>
		<header>
			...
			<span>
			<?php if(!empty($config['Gallery Title'])) { ?><?php if(!empty($config['Home Page'])) { ?><a rel="tooltip" href="<?php echo $config['Home Page']; ?>" title="<?php echo $config['Gallery Description']; ?>"><?php } ?><?php echo $config['Gallery Title']; ?><?php if(!empty($config['Home Page'])) { ?></a><?php } ?><?php } ?>
			</span></h1>
		</header>
	<?php } ?>
<?php } ?>
	...
	<?php } ?><div class="popup-gallery">
	<?php for($i = 0; $i < $number_of_images; $i++) { ?>
		<?php $image_title = str_replace('_', ' ', substr($images[$i], 0, strrpos($images[$i], '.'))); ?>
			<a rel="tooltip" href="<?php echo (empty($config['Image Size']) ? '' : $set['script name'] . '?image=') . $config['Images Dir'] . $images[$i]; ?>" data-infos="<?php echo $image_title; ?>" title="<?php echo $image_title; ?>"><img src="<?php echo $set['script name']; ?>?thumbnail=<?php echo $config['Images Dir'] . $images[$i]; ?>" alt="<?php echo $image_title; ?>"/></a>
	<?php } ?>
		</div>
...
<?php if($config['Embedded Script'] == 'off') { ?>
	<footer>
		<nav><?php if(!empty($config['Home Page'])) { ?><a rel="tooltip" href="<?php echo $config['Home Page']; ?>" title="Go to the home page">Home</a><?php } ?><?php if(!empty($config['Contact'])) { ?> • <a rel="tooltip" href="<?php echo $config['Contact']; ?>" title="Contact us">Contact</a><?php } ?><?php if(!empty($config['Imprint'])) { ?> • <a rel="tooltip" href="<?php echo $config['Imprint']; ?>" title="Imprint this page">Imprint</a> <?php } ?></nav><?php if(!empty($config['Author'])) { ?><p>Photos by <strong><?php echo $config['Author']; ?></strong> - Copyright © 2017 - All rights reserved.</p><?php } ?>
		<p>Powered by <strong>Pagemap ImageWall</strong> adapted by <a rel="tooltip" href="https://xoofoo.org/" title="XooFoo Websites"><strong>XooFoo</strong></a></p>
	</footer>
<?php } ?>
</div>
<script>
	...
	$('.popup-gallery').magnificPopup({
	  ...
	  image: {
		...
		titleSrc: function(item) {
		  return item.el.attr('data-infos') + '<small>by <?php echo $config['Author']; ?></small>';
		}
	  }
	});
});	
